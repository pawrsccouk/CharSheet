...
func addSpecialty(managedObjectContext: NSManagedObjectContext) -> Specialty {
    return NSEntityDescription.insertNewObjectForEntityForName("Specialty", inManagedObjectContext:managedObjectContext)
		as! Specialty
}
...
class Skill : NSManagedObject, XMLClient {
    ...
    func asXML() -> DDXMLElement {
        ...
        this.addAttribute( attribute(Attribute.NAME , self.name as! String) )
    ...
    func updateFromXML(element: DDXMLElement, inout error:NSError?) -> Bool {
        if !XMLSupport.validateElementName(element.name, expectedName: Element.SKILL.rawValue, error: &error) { return false }
        ...
        for attrNode in (element.attributes as! [DDXMLNode]) {
            ...
            else { return XMLSupport.setError(&error, text: "Unrecognised attribute \(attrNode.name) in skill") }
        }
        if element.childCount != 1 { NSLog("Warning: Skill has more than one child. Should be just one: \(Element.SPECIALTIES)") }
        if(element.childCount > 0) {
            for specGroup in (element.children as! [DDXMLElement]) {
                if specGroup.name == Element.SPECIALTIES.rawValue {
                    let specialties = XMLSupport.dataFromNodes(specGroup, createFunc: { addSpecialty(self.managedObjectContext!) }, error: &error)
